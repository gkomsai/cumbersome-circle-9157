import { Box, Heading, Image } from "@chakra-ui/react";
import React from "react";



const Carousol = () => {
  return (
    <Box>
      <Heading>
        Estimate tasks, set budgets, build reports & track time. All inside your
        project management tool.
      </Heading>

      <Box>
        <Box>
          <Box>
            <Image src="https://i.pinimg.com/736x/70/70/c6/7070c620d7d048b477199d44f3a12f54.jpg" />
          </Box>
          <div>Asana</div>
        </Box>
        <Box>
          <div>
            <Image src="https://blog-cdn.everhour.com/assets/images/new-design/screens/integrations/asana-primary.webp" />
          </div>
          <div>Trello</div>
        </Box>
        <Image>
          <div>
            <Image src="https://res.cloudinary.com/crunchbase-production/image/upload/c_lpad,f_auto,q_auto:eco,dpr_1/v1487968581/clbterhufowxi49prmdz.png" />
          </div>
          <div>BaseCamp</div>
        </Image>
        <Box>
          <div>
            <Image src="data:image/png;base64," />
          </div>
          <div>Jira</div>
        </Box>
        <Box>
          <div>
            <Image src="data:image/png;base64," />
          </div>
          <div>GitHub</div>
        </Box>
        <Box>
          <div>
            <Image src="https://play-lh.googleusercontent.com/9U1Zbu7jlryfcgwvWHulHzjlgMVjOy_S4rIATrF0b6Qd_VR_5yftp0dQBfi-Q5Psjg=w240-h480-rw" />
          </div>
          <div>ClickUp</div>
        </Box>
      </Box>

    
    </Box>
  );
};

export default Carousol;
